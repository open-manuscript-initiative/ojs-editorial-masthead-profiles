 ...
 *
 * Based on PKP's OJS 3.5 Editorial Masthead frontend template.
 * The only functional change is that current masthead member names link to
 * this plugin's public editorProfile route.
 *}
			...
			{foreach from=$mastheadUsers[$mastheadRole->id] item="mastheadUser"}
				<li>
					{strip}
						...
						{/if}
						<span class="name">
							<a class="editorial-masthead-profile-link" href="{url page="editorProfile" op="view" path=$mastheadUser['user']->getId()}">{$mastheadUser['user']->getFullName()|escape}</a>
