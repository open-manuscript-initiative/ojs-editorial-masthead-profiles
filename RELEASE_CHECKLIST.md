...
- [x] `version.xml` declares `1.0.8.1`.
...
- [x] Release workflow builds `editorialMastheadProfiles-1.0.8.1.tar.gz` with the required top-level `editorialMastheadProfiles` directory.
- [x] Release workflow generates the matching `.md5` checksum file.
- [ ] Run the release workflow from `main` after this PR is merged.
- [ ] Verify that both custom release assets are attached to GitHub Release `v1.0.8.1`.
